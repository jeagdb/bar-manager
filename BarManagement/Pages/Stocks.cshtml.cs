...
namespace BarManagement.Pages
{
    ...
    public class StocksModel : PageModel
    {
        ...
        public async Task<IActionResult> OnPostStocks()
        {
            ...
            var transaction = await _transactionsRepository.Insert(new Models.Transactions() { SellDate = DateTime.Now, Value = - Double.Parse((FormStock.PRICE).Replace('.', ',')), CocktailId = null });
            stock.Drink = drink;
            return Redirect("./Index");
        }
